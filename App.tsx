    ...
    <div className="flex min-h-screen bg-[#F7F7F5] text-[#37352F]">
      <aside className="w-64 border-r border-[#E9E9E7] bg-[#F7F7F5] flex-shrink-0 fixed h-full z-10 hidden md:flex flex-col justify-between">
        <div>
            ...
            <nav className="px-2 space-y-1">
              <button onClick={() => setActiveSection(SectionType.DASHBOARD)} className={`w-full flex items-center gap-3 px-3 py-2 rounded text-sm font-medium transition-colors ${activeSection === SectionType.DASHBOARD ? 'bg-[#EFEFED] text-black' : 'text-gray-500 hover:bg-[#EFEFED]'}`}>
              ...
              <button onClick={() => setActiveSection(SectionType.ONSHORE_LIQUIDITY)} className={`w-full flex items-center gap-3 px-3 py-2 rounded text-sm font-medium transition-colors ${activeSection === SectionType.ONSHORE_LIQUIDITY ? 'bg-[#EFEFED] text-black' : 'text-gray-500 hover:bg-[#EFEFED]'}`}>
                <Activity size={18} />
                {t.nav.onshore}
              </button>
              <button onClick={() => setActiveSection(SectionType.OFFSHORE_LIQUIDITY)} className={`w-full flex items-center gap-3 px-3 py-2 rounded text-sm font-medium transition-colors ${activeSection === SectionType.OFFSHORE_LIQUIDITY ? 'bg-[#EFEFED] text-black' : 'text-gray-500 hover:bg-[#EFEFED]'}`}>
                <Globe size={18} />
                {t.nav.offshore}
              </button>
              <button onClick={() => setActiveSection(SectionType.FED_POLICY)} className={`w-full flex items-center gap-3 px-3 py-2 rounded text-sm font-medium transition-colors ${activeSection === SectionType.FED_POLICY ? 'bg-[#EFEFED] text-black' : 'text-gray-500 hover:bg-[#EFEFED]'}`}>
                <Landmark size={18} />
                {t.nav.fed}
              </button>
              <button onClick={() => setActiveSection(ExtendedSectionType.CALENDAR)} className={`w-full flex items-center gap-3 px-3 py-2 rounded text-sm font-medium transition-colors ${activeSection === ExtendedSectionType.CALENDAR ? 'bg-[#EFEFED] text-black' : 'text-gray-500 hover:bg-[#EFEFED]'}`}>
                <Calendar size={18} />
                {t.nav.calendar}
              </button>
            </nav>
        ...
        <div className="p-4 border-t border-[#E9E9E7] space-y-3">
             <button onClick={handleGenerateVideo} disabled={generatingVideo} className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-gradient-to-r from-gray-800 to-black text-white rounded shadow-sm text-xs font-medium hover:opacity-90 transition-opacity disabled:opacity-50">
                {generatingVideo ? (
      ...
      <main className="flex-1 md:ml-64 p-8 max-w-7xl mx-auto w-full relative">
        ...
        {activeSection === SectionType.DASHBOARD && (
            <>
                <div className="mb-8">
                  <BubbleMap onshore={ONSHORE_INDICATORS} offshore={OFFSHORE_INDICATORS} onSelect={handleBubbleSelect} selectedId={selectedChart?.id} lang={lang} />
                </div>
        ...
        {activeSection === SectionType.ONSHORE_LIQUIDITY && (
            <>
             <SectionAnalysis title={t.headers.onshore.title} indicators={ONSHORE_INDICATORS} colorTheme="blue" lang={lang} />
             <GroupedIndicators indicators={ONSHORE_INDICATORS} onSelect={handleBubbleSelect} selectedId={selectedChart?.id} lang={lang} />
            </>
        )}

        {activeSection === SectionType.OFFSHORE_LIQUIDITY && (
            <>
             <SectionAnalysis title={t.headers.offshore.title} indicators={OFFSHORE_INDICATORS} colorTheme="indigo" lang={lang} />
             <GroupedIndicators indicators={OFFSHORE_INDICATORS} onSelect={handleBubbleSelect} selectedId={selectedChart?.id} lang={lang} />
            </>
        )}
        ...
        {activeSection === SectionType.FED_POLICY && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                        <div className="px-6 py-4 bg-gray-50 border-b border-gray-200 flex items-center gap-2">
                             <CalendarDays size={18} className="text-gray-500"/><h3 className="font-semibold text-gray-700">{t.fedTable.title}</h3>
                        </div>
                                ...
                                {FED_EVENTS.map((event, idx) => (
                                    <tr key={idx} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 w-24">
                                            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${event.type === 'Meeting' ? 'bg-blue-100 text-blue-800' : event.type === 'Speech' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800'}`}>
                                                {event.type}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-right">
                                            <div className="flex items-center justify-end gap-1">
                                                <div className={`w-2 h-2 rounded-full ${event.impactLevel === 'High' ? 'bg-red-500' : event.impactLevel === 'Medium' ? 'bg-yellow-500' : 'bg-green-500'}`}></div>{event.impactLevel}
                                            </div>
                                        </td>
                                    </tr>
                                ))}
                ...
                <div className="space-y-4">
                     {FED_INDICATORS.map(ind => <IndicatorCard key={ind.id} indicator={ind} selected={selectedChart?.id === ind.id} onSelect={() => handleBubbleSelect(ind)} lang={lang} />)}
                </div>
            </div>
        )}
        ...
        {selectedChart && activeSection !== ExtendedSectionType.CALENDAR && (
            ...
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2"><TrendingUp size={20} className="text-blue-600"/>{lang === 'zh' ? selectedChart.name : selectedChart.nameEn} ({selectedChart.code})</h2>
                <div className="text-xs font-mono bg-gray-100 px-2 py-1 rounded text-gray-500">{UI_TEXT[lang].subCategories[selectedChart.subCategory]}</div>
                <button onClick={() => setSelectedChart(null)} className="text-gray-400 hover:text-gray-600"><X size={20} /></button>
            </div>
            <div className="space-y-2">
                <ChartComponent data={selectedChart.history} dataKey1={selectedChart.code} color1="#2563EB" height={250} />
                <div className="mt-2 text-xs text-gray-500 bg-gray-50 p-3 rounded border border-gray-100">{lang === 'zh' ? selectedChart.description : selectedChart.descriptionEn}</div>
            </div>
            </div>
        )}
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                <div className="bg-white rounded-xl shadow-2xl overflow-hidden max-w-2xl w-full">
                    <div className="p-4 border-b border-gray-100 flex justify-between items-center">
                        <h3 className="font-bold text-gray-800 flex items-center gap-2"><Video size={18} className="text-purple-600"/>{t.actions.generateVideo}</h3>
                        <button onClick={() => setVideoUrl(null)} className="text-gray-400 hover:text-gray-600"><X size={20} /></button>
                    </div>
                    <div className="bg-black aspect-video flex items-center justify-center"><video src={videoUrl} controls autoPlay className="w-full h-full"/></div>
                    <div className="p-4 bg-gray-50 flex justify-end"><a href={videoUrl} download="liquidity-watch-demo.mp4" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors"><Download size={16} />{t.actions.downloadVideo}</a></div>
                </div>
            </div>
        )}
        ...
        {videoError && (
            <div className="fixed bottom-8 right-8 bg-red-50 text-red-600 px-4 py-3 rounded-lg border border-red-200 shadow-lg text-sm z-50 animate-in slide-in-from-right">
                {videoError}<button onClick={() => setVideoError(null)} className="ml-2 underline">{t.actions.close}</button>
            </div>
        )}
      </main>
    </div>
